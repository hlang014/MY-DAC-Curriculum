...
-- unique values from specific column
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT *
FROM humanresources.employee;
WHERE birthdate LIKE '1969-01-29%'; -- not supposed to work as data is not a string

-- But what if you know the number of letters in the firstname?

SELECT *
FROM person.person
WHERE firstname LIKE 'J___'; -- shows names starting with J and has 4 letters
...
-- shows names that contain letter A (not just starting with A)
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- Average vacation hours is lesser from those married in both genders.
...
SELECT
	jobtitle,
	AVG(sickleavehours) AS average_sick_leavehours
FROM humanresources.employee
GROUP BY jobtitle
...
FROM humanresources.employee
ORDER BY birthdate ASC
OFFSET 10; -- exclude top 10 oldest employees 
...
SELECT *
FROM humanresources.employee
WHERE gender ='M' -- removes female entries
-- WHERE modifieddate = '2024-10-29'  
LIMIT 10;
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- JOINS: LEFT 
-- values from left table will be included with intersection 
...
SELECT *
FROM sales.salesorderdetail; -- when joining 2 tables tgt, column names can be different as long as values are matching 

SELECT 
	product.productid,
	product.name AS productname, -- write last
	COALESCE(SUM(salesorderdetail.orderqty),0) AS totalsalesquantity -- look for null values, given value of 0
...
FROM humanresources.employee AS employee -- alias
LEFT JOIN person.person AS person 
	ON employee.businessentityid = person.businessentityid -- left table
...
SELECT *
FROM sales.customer; -- .person.businessentityid = sales.customerid (e.g.)
...
-- A6: 
/*
It depends on the relationship that both the tables present from the left join share.
